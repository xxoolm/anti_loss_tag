...
3. 打开 diagnostics 查看连接分类：
   - `connection_error_classification`
   - `connection_error_type`
   - `connection_manager.acquire_timeout`

   如果 `slot_timeout` 比例高，优先减少同时间段的按钮操作和后台轮询任务。
...
2. 设备电量低
3. 设备超出范围或存在信号干扰
4. ESPHome代理槽位不足（如果使用ESPHome蓝牙代理）

**诊断步骤**：

1. **查看信号强度值**：
    - 进入"开发者工具" → "状态"
    - 查找您的设备名称，找到对应的信号强度传感器
    - 查看 `sensor.{设备名}_rssi` 的值（entity_id可能是ASCII格式）
    - 正常范围：-50 到 -90 dBm
...
3. **查看 diagnostics 关键字段**：
   - `operation_queue_size`：是否长期堆积
   - `battery_defer_count`：后台轮询是否在高负载时主动让路
   - `last_battery_sleep_reason`：是否持续为 `defer_for_foreground` 或 `conn_mgr_congested`
   - `battery_read_busy`：是否长时间处于 true

**解决方法**：

1. 减少 `battery_poll_interval_min` 到更合理的值（例如120分钟）

**注意**：默认值是360分钟（6小时），如果觉得更新不够频繁，可以调整为60-120分钟。
...
1. **检查设备是否在范围内**：
    - 查看状态：`binary_sensor.{设备名}_in_range`（entity_id可能是ASCII格式）
    - 如果显示"关"，说明设备超出范围
...
3. 检查蓝牙扫描器状态：
```bash
# 查看Home Assistant蓝牙扫描器状态
grep "scanner\|BLE" home-assistant.log | tail -20
```

**注意**：RSSI通过蓝牙广播自动更新，不需要配置轮询间隔。
...
1. **检查实体ID**：
    - 进入"开发者工具" → "状态"
    - 搜索您的设备名称
    - 确认entity_id正确（如 `binary_sensor.my_tag_far_away`）
    - **注意**：实际entity_id可能是ASCII格式，取决于设备名称和HA转换规则
...
automation:
  - alias: "调试标签远离"
    trigger:
      - platform: state
        entity_id: binary_sensor.my_tag_far_away  # 实际entity_id可能不同
        to: "on"
    action:
      - service: logbook.log
        data:
          message: "标签已远离：{{ trigger.from_state.state }} → {{ trigger.to_state.state }}"
          message: "标签远离自动化已触发"
```
