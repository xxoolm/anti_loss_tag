...
1. **不保证100%防丢效果**：本集成不应作为唯一的安全措施，BLE连接受距离、干扰、电池电量等因素影响可能失效
2. **资产保护限制**：请勿依赖本集成保护贵重物品、重要资产或敏感信息
3. **定期检查设备**：请定期检查设备电量、连接状态和报警功能
4. **配合其他措施**：强烈建议配合其他防丢措施使用（如GPS追踪器、多重标签等）
5. **环境因素**：电磁干扰、物理障碍物、设备兼容性可能影响性能
6. **数据隐私**：BLE设备地址和连接信息会被记录在Home Assistant日志中

**已知限制**：

- BLE有效距离通常在10-30米，受环境影响
- 设备电池耗尽时无法报警
- Home Assistant重启或服务中断期间防丢功能暂停
- 某些BLE适配器可能存在兼容性问题
...
**已知问题**:
- 部分低质量 BLE 适配器在高并发下仍可能出现超时
- ESPHome 蓝牙代理槽位紧张时可能导致后台轮询延后
- 测试环境若缺少 `homeassistant` 依赖将无法执行本地 pytest
...
-  **智能重连**: 指数退避策略，避免连接风暴
-  **串行操作队列**: 每设备独立优先级队列，避免并发 GATT 冲突
-  **前台优先调度**: 报警按钮优先，后台电量轮询自动让路
-  **自适应轮询**: 根据连接拥塞和超时率动态调整后台读取节奏
-  **增强诊断**: 输出连接分类、队列长度、槽位等待和轮询原因
